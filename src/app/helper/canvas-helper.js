...
export const manageLineDrawing = (
  svg,
  g,
  overlay,
  enableLineDrawing,
    ...
    overlay.on("click", (event) => {
      ...
      if (!lineStartPoint) {
        ...
        if (drawingType === "trend") {
          ...
          const dataXStart = currentTransform
            .rescaleX(x)
            .invert(lineStartPoint.x);
          ...
          overlay.on("mousemove", (event) => {
            if (startPoint && enableLineDrawing) {
              ...
              const endYRight =
                startPoint.y + (width - startPoint.x) * Math.tan(theta);
...
// Genearting the Line Chart Elements 

const defineChartElements = (svg, g, width, height, x, y) => {
  g.selectAll("*").remove();
  ...
  return { xAxis, yAxis, yAxisGrid, yAxisGroup, zoomRect, overlay };
};
...
export const generateLineChart = (
  ...
  d3.select(".link-settings-container").on("click", handleToolContainerClick);
  ...
  const zoom = d3
    ...
    .on("zoom", (event) => {
      ...
      g.selectAll(".dot").attr("cx", (d) => newX(d.Date));
      linePath.attr(
        "d",
        line.y((d) => newY(d.Close)).x((d) => newX(d.Date))
      );
      ...
      g.select("#movingAverageLine").attr(
        "d",
        movingAverageLine(movingAverageData)
      );

      exponentialLine.x((d) => newX(d.date));
      g.select("#expmovingAverageLine").attr(
        "d",
        exponentialLine(exponentialMovingAverageData)
      );
    });
...
export const generateCandleStickChart = (
  svg,
  g,
  data,
  width,
  ...
  const xAxisGrid = d3.axisBottom(x).tickSize(-height).tickFormat("").ticks(6);
  ...
  const throttledZoom = _.throttle((event) => {
    ...
    g.select(".x-axis").call(xAxis.scale(newX));
    bars
      .attr("x", (d) => newX(new Date(d.Date)) - newWidth / 2)
      .attr("width", newWidth);
    lines
      .attr("x1", (d) => newX(new Date(d.Date)))
      ...
      .on("mousemove", function (event) {
        ...
        if (hoveredData) {
          ...
          ohlcTooltip
            .style("display", "block")
            .html(
              `O: ${openValue} H: ${highValue} L: ${lowValue} C: ${closeValue}`
            )
  ...
  const zoom = d3.zoom().scaleExtent([1, 20]).on("zoom", throttledZoom);
